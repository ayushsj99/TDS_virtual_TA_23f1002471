...
def describe_image_with_gemini(image_path):
    try:
        ...
        response = client.models.generate_content(model="gemini-2.0-flash-lite", contents=["Summarize the key visual details of this image in 1-3 sentences. Focus on objects, text, layout, or any technical diagrams visible.", img])
    ...
    try:
        with open(filepath, "w", encoding="utf-8") as f:
            for post in posts:
                ...
                post_number = post.get("post_number", "Unknown Post Number")
                ...
                reply_to_username = post.get("reply_to_user.username", "Unknown User")
                ...
                f.write(f"## Post by {author} of username: {username} on {created_at}\n\n")
                f.write(f"**Post Number:** {post_number}\n")
                f.write(f"**Reply to Post Number:** {reply_to}\n")
                f.write(f"**Reply to Username:** {reply_to_username}\n")
